...
Use the `generate_diagram` tool to create a FigJam board named `interaction-state-machine — [input]`, using Mermaid `stateDiagram-v2` syntax: states as nodes, labeled transitions for each event (`StateA --> StateB: event`), guards noted in the transition label where relevant, and `[*]` for initial/final pseudo-states. If more than one Figma plan/team is available, ask which to use before generating.
