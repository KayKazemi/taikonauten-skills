...
Use the `generate_diagram` tool to create a FigJam board named `research-journey-map — [input]`, using Mermaid flowchart syntax (LR direction, all shape/edge text quoted): one subgraph per journey stage in sequence, each containing nodes for that stage's key actions, emotional state, and pain points/opportunities; connect stages left to right to show progression. If more than one Figma plan/team is available, ask which to use before generating.
