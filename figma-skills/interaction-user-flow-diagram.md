---
name: interaction-user-flow-diagram
...
Use the `generate_diagram` tool to create a FigJam board named `interaction-user-flow-diagram — [input]`, using Mermaid flowchart syntax (LR direction, all shape/edge text quoted) mapped to the element vocabulary above: entry point → circle `((...))`, screen/page → rectangle `[...]`, decision → diamond `{...}`, action → rounded rectangle `(...)`, system process → subroutine `[[...]]`, end point → stadium `([...])`. Avoid double/triple-circle shapes — not reliably supported by the renderer. If more than one Figma plan/team is available, ask which to use before generating.
