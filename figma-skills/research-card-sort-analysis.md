---
name: research-card-sort-analysis
...
Use the `figma-use` skill to materialize this as a new frame on the current page, named `research-card-sort-analysis — [input]`, built as labeled columns (one per resulting IA category), each containing small colored rectangles (simulating cards) listing the items sorted into that category, plus a summary panel noting ambiguous or inconsistently-sorted items. Build incrementally and verify with a screenshot before finishing.
